---
title: 第十五章 Iterator 和 for...of 循环
tags: ES6
categories: ECMAScript 6 入门
abbrlink: c1195842
date: 2020-06-10 20:56:54
---
对于JavaScript 原有的数组（`Array`）和对象（`Object`）以及ES6 添加的`Map`和`Set`，甚至组合使用它们，定义自己的数据结构，比如数组的成员是`Map`，`Map`的成员是对象。需要一种统一的接口机制，来处理所有不同的数据结构。<!-- more -->
